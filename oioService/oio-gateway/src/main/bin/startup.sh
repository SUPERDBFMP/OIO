...
if [ -f $current_path/bin/adapter.pid ] ; then
...
CANAL_OPTS="-DappName=oio-gateway"
...
$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $CANAL_OPTS -jar oio-gateway.jar 1>>/dev/null 2>&1 &
echo $! > $current_path/bin/admin.pid
